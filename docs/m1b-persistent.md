...
| `boot-m1b-v7.img`（**当前部署**，含 `recovery_dtbo`） | 手机 sdcard `lmi-m1b/`；`/data/local/lmi-dualboot/` | v6 + `recovery_dtbo`（487,424 B，recovery 引导必需，T1-03）；sha256 `754b63b4…`（55,029,760 B）；overlay v2 `db760fec…`（5,991,807 B） |
| `boot-m1b-v6.img`（保留对照；**勿用于 recovery 部署**） | 同上 | v5 + overlay v2（修补文件入 overlay，版本 `m1b-wifi-v2`）；缺 `recovery_dtbo` → recovery 引导落 fastboot；sha256 `346343b3…`（54,546,432 B） |
