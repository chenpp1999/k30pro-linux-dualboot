...
> 状态：2026-09-13 方式 A 实机验收通过（A1–A5），见 docs/acceptance/m0-2026-09-13.md。
...
| 产物 | 字节数 | SHA-256 |
|---|---|---|
| `boot-m0.img` | 48,615,424 | `168ed21dd9968d91086cef9f3b80d41a86d470a227b52587b0114f4539ac090a` |
| `initramfs.cpio.gz` | 4,476,615 | `e29a9b24938252f0de336613d258194730e8001e9638ddc539be3d47d07665a0` |

> 2026-09-14 重建（含 issue #1–#11 全部修复：NCM/LF/显示接管 + 本轮
> #5–#11）：eventdump 多设备 poll；`attest-ramboot`
> 同步刷新 `.sha256` 清单；`restore-twrp` 增加 `--dry-run` 与写后校验；构建
> 可复现打包与 busybox 库随行；telnet 默认关闭（`lmi_telnet=1` 显式启用）；
> display 平面计数夹取；文档一致性修订。
> 注：产物哈希随重建变化（构建环境/时间），每次重建更新本表；某次验收对应的
> 构建哈希见 `docs/acceptance/`。
...
7. 等待 30–60 秒。宿主侧应出现一个新网卡（NCM，如 `UsbNcm Host Device`；RNDIS 回退）；
8. 宿主配置静态 IP：
...
9. `ssh root@172.16.42.1`（密码见构建输出）；无宿主静态 IPv4 时可用
   **IPv6 链路本地**：先 `ping -6 ff02::1%<ifindex>` 触发邻居发现，再从
   ...
   `lmi_telnet=1` 重新 boot（经 login 认证，issue #9）；
10. 收集证据（在宿主执行）：
...
1b. **把备份复制到宿主/外部存储（§5 救援需要宿主侧副本）**：

   ```sh
   adb shell su -c 'cp /data/local/lmi-dualboot/*.img /data/local/lmi-dualboot/*.sha256 /sdcard/Download/phone-server/lmi-m0/backup/ && ls -l /sdcard/Download/phone-server/lmi-m0/backup/'
   adb pull /sdcard/Download/phone-server/lmi-m0/backup ./lmi-backup/
   ```
...
2. **门禁：同一镜像必须先经方式 A 实机启动成功，并生成 attestation**
   ...
   产物：`boot-m0.img.ramboot-ok`（并同时刷新 `boot-m0.img.sha256` 清单，
   `to-linux` 预检依赖它，issue #6）。**方式 A 未成功前不得执行本步。**
...
- 二级救援（可选）：宿主侧有备份副本时（§4 第 1b 步），可
  `fastboot boot recovery-twrp.img` RAM 启动 TWRP；也可
  `fastboot boot boot-m0.img` 先确认 Linux 镜像本身能否启动，再决定恢复动作。
...
用宿主侧备份重刷 `boot`（`fastboot flash boot boot-android.img`，备份副本见
§4 第 1b 步；手机内原件在 `/data/local/lmi-dualboot/`），并开 P1 缺陷。
