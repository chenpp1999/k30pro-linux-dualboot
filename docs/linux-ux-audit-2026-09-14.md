...
- 网络：WiFi（SSID 为按机信息，不入仓）正常；NCM `172.16.42.1` 正常；无蓝牙。
- 音频：无声卡（仅 timer）。
...
### 6.6 中文模式退格修复（2026-09-15）

- 现象：中文输入提交后，OSK 的退格只能删拼音缓冲，删不掉终端里的字。
- 根因：`keyboard_commit_text()` 自建 surrounding text → 之后退格走
  `delete_surrounding_text` 路径，而终端（补丁 0001）对该事件是空实现。
- 修复：① 键盘不再自建 surrounding text（客户端没提供就保持 NULL，终端继续
  走 BackSpace keysym，可无限删除）；② 补丁 0008 给终端实现
  `delete_surrounding_text`（按字符近似字节长度）作兜底。
- 验证：`nihao`→提交"你好"（截图 `/# 你好`）→ 两次退格 → 终端回到 `/#`。
...
### 6.8 字体"又变白框"的根因与加固（2026-09-15）

- 现象：M3 迁移后某次开机，终端里的中文又显示为白框（键盘自绘的中文正常）。
- 根因（两件事叠加）：
  1. **冷启动早期 fontconfig 正在重建缓存**：开机时系统时钟还是 swclock 时间戳
     （可能落在过去），而字体文件的 mtime 较新，fontconfig 判定缓存过期并在
     客户端启动的窗口内重建 → 最先启动的 cairo 客户端可能拿不到 CJK 字形；
  2. **weston 崩过一次**（rc=134/SIGABRT，wrapper 连续 3 次失败后**自弃退出**），
     桌面整体停摆——用户看到的"没字体"其实是合成器都没了。
- 修复（`tools/m1/m1b/usr/sbin/m1-weston`，overlay 内）：
  1. 启动 weston 前执行 `fc-cache -f`（3 个字体的开销可忽略），保证任何客户端
     启动时缓存已就绪；
  2. 重试策略改为**永不自弃**：weston 崩溃/启动失败都持续重试（退避 3 s，
     连续 20 次失败后转 30 s 长退避），避免合成器缺失导致"黑屏/无字体"。
- 验证：重启后终端输入中文正常（截图 `/# 你好`）；wrapper 日志显示正常重启。
- 另记：terminal 的 `--font=` 命令行选项**不接受含空格的名字**（toytoolkit 解析），
  所以 CJK 终端字体只能走 weston.ini 的 `[terminal] font=`（或加 fontconfig 别名），
  不要试图在 wrapper 里加 `--font="WenQuanYi Zen Hei Mono"`。
