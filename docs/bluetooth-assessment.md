...
### 6c.11 P3 第七轮（2026-09-18）：扬声器无声定性为**硬件故障**（人耳 + 功放内置测试音）

§6c.10 的待确认项已有答案：**Android 扬声器同样无声，且问题在硬件层（喇叭/功放输出），
不是 Linux 软件回归。**

**人耳实测（用户本人在场）**：

- Linux：听筒单独播 1 kHz（3 次）→ **听到**（顶部）；扬声器路径单独播（听筒已关，
  FE MultiMedia5 + S24_3LE + `Playback 9 Volume=8192`）→ **完全没声**。
- Android：正常放音乐/视频/铃声，媒体音量已程序化拉满 15/15、蓝牙无音频设备、
  音频策略 `Devices: speaker(2)` 正常 → **只有听筒有声，底部扬声器无声**。
- 用户回顾：**以前扬声器是好的**，现在两端都哑。

**决定性测试：TFA9874 内置正弦测试发生器**（`tfa9874_tfafieldnames.h`：
`tsig_freq_lsb=0xd10f`、`tsig_freq_msb=0xd202`、`inject_tsig=0xd230`、
`tsig_gain=0xd283`）。该发生器**绕过 SoC/MI2S 数据通路**，直接把测试音注入功放输出级：

- 空闲时注入（先手写 `0x00=0x0018` 拉起 PWDN/AMPE/DCA）：5 个频率 × 2 s → 无声。
- **播放中注入**（`0x00=0x0018`、`0x11=0xe2fa` 时钟已锁定、PLLS/CLKS=1）：同样
  5 个频率 → **一点声音都没有**。
- 排除了"功放被软件静音"：`MANMUTE=0`、`0x13` 的 OCP 位是 **OCPOK（=1 正常）**、
  驱动 dmesg 无任何错误、`reset-gpio`（tlmm 114）在 probe 时正常复位、irq-gpio 100 正常。
- 结论：**功放输出级 → 喇叭这一段没有响应**（喇叭线圈/连接线/功放输出级硬件故障），
  与 SoC 数据、路由、容器、两个 OS 的驱动都无关。

**可能原因（无法证伪/证实）**：本项目此前的 bring-up 中有过多轮**满幅 1 kHz 长音
（FE 音量拉满）**以及**播放中翻转 TDM 寄存器的实验**，这类操作可能使喇叭过流/过热
（TDM 实验尤其可能向喇叭灌直流）；也不能排除物理损伤（摔/进水/排线松脱）。
现有证据无法区分，但"两端同时哑 + 内置测试音不响"只能落在硬件。

**后续（不在本项目软件范围内）**：更换底部扬声器模块（K30 Pro 的底部喇叭在副板上，
维修点可换）；若换后仍无声，则怀疑 TFA9874 输出级/主板，需要板级维修。换好喇叭后，
**Linux 侧已经就绪**（声卡/路由/麦克风/听筒都已在 v24 + 修好的 rootfs 上验证），
无需再改软件。在维修之前，Linux 可用听筒或蓝牙/USB 音频。
