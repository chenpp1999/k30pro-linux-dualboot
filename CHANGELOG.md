...
### Fixed
- **扬声器无声定性为硬件故障（2026-09-18，第七轮，人耳 + 功放内置测试音）**：
  Android 侧同样只有听筒有声（媒体音量 15/15、无蓝牙音频、策略 `Devices: speaker`），
  用户确认"以前扬声器是好的"；TFA9874 **内置正弦测试发生器**（`0xd1`/`0xd2` 的
  `tsig_freq`/`gain`/`inject_tsig`，绕过 SoC/MI2S）在**播放中**（`0x11=0xe2fa`，
  时钟已锁定）注入 5 个频率也**完全无声** → 功放输出级→喇叭硬件故障（OCPOK 正常、
  驱动无错、复位 GPIO 正常）。可能与此前满幅长音/播放中翻转 TDM 寄存器的实验有关
  （无法证实）；后续为硬件维修（换底部扬声器模块）。详见
  `docs/bluetooth-assessment.md` §6c.11。
